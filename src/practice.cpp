...
Practice::Practice(QWidget *parent) : QWidget(parent), nh(), num_answers(0)
{
    ...
    pub_answers = nh.advertise<master_thesis_program::Answers>("/practice/answers", 1);
}

void
Practice::mousePressEvent(QMouseEvent *e)
{
    if(e->button() == Qt::RightButton){
        if(stacked_widget->currentIndex() == info_index){
...
int main(int argc, char **argv)
{
    ...
    Practice *practice_app = new Practice(window);
    layout->addWidget(practice_app);
